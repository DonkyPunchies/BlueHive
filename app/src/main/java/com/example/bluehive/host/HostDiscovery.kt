 ...
 * app that launched it to supply credentials over the io.bluehive.host contract,
 * and names no host in particular.
 *
 * Resolution order (most authoritative first):
 *   1. The launching app (activity referrer) — the app that fired ACTION_LAUNCH —
 *      but only if it actually exposes the IBlueHiveHost bind service. This lets
...
object HostDiscovery {
    ...
    fun resolveInstalledHost(context: Context): String? {
        val intent = Intent(BlueHiveHostContract.ACTION_BIND_HOST)
        ...
        return when {
            pkgs.isEmpty() -> {
                Log.w(TAG, "No installed host exposes ${BlueHiveHostContract.ACTION_BIND_HOST}")
                null
            }
            pkgs.size == 1 -> {
    ...
    private fun exposesHostService(context: Context, pkg: String): Boolean {
        val intent = Intent(BlueHiveHostContract.ACTION_BIND_HOST).setPackage(pkg)
